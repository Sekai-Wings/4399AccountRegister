...
由于该项目使用了 **onnxruntime_go** 进行图形验证吗识别，所以你需要先从 [ONNX Runtime仓库](https://github.com/microsoft/onnxruntime) 下载最新最热的ONNX Runtime，然后提取出onnxruntime so/dll，放入该项目文件夹，然后在config.json中填入相对路径，修改想要的参数，随后下载一下库run即可。
